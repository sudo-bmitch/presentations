...
.pic-80[.pic-rounded-10[![Swarm HA Architecture](img/swarm-diagram.png)]]
...
- Now pull images directly, /ubuntu doesn't work, the private one does, hmm
- We need `/library/ubuntu` since that's the underlying path of the official images
...
- Lets try scaling the builders, each pulling from the same cache
